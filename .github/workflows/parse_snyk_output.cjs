...
async function createJiraTicket(vulnerability) {
  ...
  let jqlQuery = `project = "${process.env.JIRA_PROJECT_KEY}" AND summary ~ "MFP - SNYK ${vulnerability.title}" AND created >= startOfDay("-60d") AND status NOT IN ("Closed", "Cancelled")`;

...
- SLACK_WEBHOOK: This publishes to the `macpro-mdct-mfp-alerts` channel. Alerts published there are for deploy or test failures to the `main`, `val`, or `production` branches. It also reports weekly statuses for LaunchDarkly flags in `production`.
